...
func testUserToManyProducts(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadProducts(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpProducts(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `GivenName`: `character varying`, `FamilyName`: `character varying`, `DisplayName`: `character varying`, `MailAddress`: `character varying`, `UserRole`: `integer`, `UserStatus`: `integer`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
