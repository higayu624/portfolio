...
func testProductToManyTechnicalStacks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, technicalStackDBTypes, false, technicalStackColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, technicalStackDBTypes, false, technicalStackColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.TechnicalStacks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadTechnicalStacks(ctx, tx, false, (*[]*Product)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TechnicalStacks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.TechnicalStacks = nil
	if err = a.L.LoadTechnicalStacks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TechnicalStacks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProductToManyAddOpTechnicalStacks(t *testing.T) {
	var err error
	...
	foreigners := []*TechnicalStack{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, technicalStackDBTypes, false, strmangle.SetComplement(technicalStackPrimaryKeyColumns, technicalStackColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*TechnicalStack{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddTechnicalStacks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.TechnicalStacks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.TechnicalStacks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.TechnicalStacks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testProductToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Product)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testProductToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	productDBTypes = map[string]string{`ID`: `integer`, `UserID`: `integer`, `ProductLink`: `character varying`, `Title`: `character varying`, `SubTitle`: `character varying`, `Exposition`: `character varying`, `AppealPoint`: `character varying`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_              = bytes.MinRead
)
